...
<script>
    ...
    function saveDescription() {
        ...
        row.cells[10].innerHTML = `<span class="description-span">${description}</span><button class="btn btn-sm btn-success" type="button" onclick="openDescriptionModal(this)">Edit Description</button>`;
    ...
    function submitProducts() {
        ...
        rows.forEach(row => {
            ...
            imagesFiles.forEach((imageFile, imgIndex) => {
                const validExtensions = ['image/jpeg', 'image/png', 'image/gif', 'image/svg']; // Allowed file types
        ...
        if (products.length != pricesArr.length) {
            html = `<div class="alert alert-warning alert-dismissible alert-label-icon label-arrow fade show material-shadow" role="alert">
                        <i class="ri-alert-line label-icon"></i><strong>Warning</strong> - Please Add Prices of All Products. If price is alredy added then check the image extesion(.jpeg, .pnd, .gif) and size.
                        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                    </div>`
            $('#alert').html(html)
            return
